...
from action.executor import run_user_code, save_session_vars
...
async def execute_step(step_id, code, ctx, session, multi_mcp, variant_used: str = ""):
    result = None
    try:
        latest_perception = ctx.get_latest_perception() if ctx else None
        if ctx and ctx.original_query:
            save_session_vars(ctx.session_id, {"original_query": ctx.original_query})
        if latest_perception:
            save_session_vars(ctx.session_id, {"perception": latest_perception})
...
async def execute_step_with_mode(step_id, code_variants, ctx, mode, session, multi_mcp):
    if mode == "parallel":
        ...
        if result and result.get("status") == "error":
            ...
            log_error(f"↳ Error: {last_error}")

        error_detail = f" All variants failed. Last error: {last_error}" if last_error else " All variants failed."
        return {"status": "error", "error": f"All fallback variants failed.{error_detail}"}
