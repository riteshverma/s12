...
def fix_unbalanced_single_quotes(code: str) -> str:
    """Best-effort fix for lines with unbalanced single quotes."""
    fixed_lines = []
    updated = False
    for line in code.splitlines():
        ...
        if line.count("'") % 2 == 1:
            ...
            last_idx = line.rfind("'")
            if last_idx != -1:
                line = line[:last_idx] + '"' + line[last_idx + 1:]
        fixed_lines.append(line)
    if updated:
        log_error("Fixing unbalanced single quotes in code...", symbol="⚠️ ")
    return "\n".join(fixed_lines)
...
async def run_user_code(code: str, multi_mcp, session_id: str = "default_session") -> dict:
    ...
    try:
        ...
        try:
            ...
            repaired_code = fix_unbalanced_single_quotes(cleaned_code)
            if repaired_code != cleaned_code:
        ...
        if isinstance(returned, dict):
            ...
            def normalize_form_data(value):
                if not isinstance(value, dict):
                    return None
                key_map = {
                    "course (what course is he/she in?)": "course",
                    "email id": "email",
                    "master's name": "master",
                    "which course is he/she taking?": "course_taking",
                    "date of birth": "dob",
                ...
                for raw_key, raw_val in value.items():
                    ...
                    if lookup in key_map:
                        normalized[key_map[lookup]] = raw_val
                return normalized or None

            # If any returned dict looks like form data, add a safe alias
            for key, value in list(result_value.items()):
                normalized = normalize_form_data(value)
                if normalized:
                    result_value[f"{key}_simple"] = normalized
